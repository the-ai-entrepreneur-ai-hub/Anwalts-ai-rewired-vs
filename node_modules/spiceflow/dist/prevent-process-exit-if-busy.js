...
export function preventProcessExitIfBusy(options = {}) {
    ...
    async function handleShutdown(signal) {
        ...
        console.log(`[${new Date().toISOString()}] ${signal} signal received for graceful shutdown`);
        console.log(`[${new Date().toISOString()}] Waiting for ${inFlightRequests} in-flight request(s) to complete...`);
        ...
        if (inFlightRequests > 0) {
            console.log(`[${new Date().toISOString()}] Shutdown timeout reached after ${elapsedSeconds}s; ${inFlightRequests} request(s) still in progress`);
            console.log(`[${new Date().toISOString()}] Forcing shutdown with exit code 1`);
        }
        else {
            console.log(`[${new Date().toISOString()}] All requests completed successfully after ${elapsedSeconds}s`);
    ...
    if (typeof process !== 'undefined' && process.prependListener) {
        ...
        console.log(`[${new Date().toISOString()}] Graceful shutdown handlers registered for SIGINT and SIGTERM`);
    }
...
//# sourceMappingURL=prevent-process-exit-if-busy.js.map
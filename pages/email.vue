    ...
    <div v-if="currentView === 'consent'" class="min-h-screen bg-white flex items-center justify-center p-4 sm:p-6 lg:p-8 relative">
      ...
      <div class="max-w-sm sm:max-w-md w-full mx-auto">
        <div class="text-center mb-6 sm:mb-8">
          ...
          <h1 class="text-xl sm:text-2xl font-semibold text-gray-900 mb-2">Connect Your Email</h1>
          <p class="text-gray-600 text-sm sm:text-base">Securely connect your professional email account</p>
        </div>
        ...
        <div class="space-y-3 mb-6">
          ...
          >
            ...
            <span class="font-medium text-gray-700">Continue with Gmail</span>
          </button>
          ...
          >
            ...
            <span class="font-medium text-gray-700">Continue with Outlook / Microsoft 365</span>
          </button>
        </div>
        ...
        <div class="bg-gray-50 rounded-xl p-4 space-y-3 mb-6">
          <label class="flex items-start gap-3 cursor-pointer">
            ...
            <span class="text-sm text-gray-700 flex-1">
              I agree to connect my email via OAuth 2.0 secure authentication
            </span>
          </label>
        ...
        <div class="flex items-center justify-center gap-4 text-sm">
          <a href="/privacy" class="text-gray-600 hover:text-gray-900 underline">Privacy Policy</a>
          <span class="text-gray-300">•</span>
          <a href="/terms" class="text-gray-600 hover:text-gray-900 underline">Data Use</a>
          <span class="text-gray-300">•</span>
          <a href="#" class="text-gray-600 hover:text-gray-900 underline">Revoke Access</a>
        </div>
        ...
        <div v-if="loading" class="mt-6 flex items-center justify-center">
          ...
          <span class="ml-2 text-gray-600">Connecting securely...</span>
        </div>
      </div>
    </div>
          ...
          <div class="flex items-center justify-between flex-wrap gap-3">
            <div class="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
              ...
              <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                ...
                <h1 class="text-lg sm:text-xl font-medium text-gray-900 truncate">Email Portal</h1>
              </div>
            ...
            <div class="flex items-center gap-2 sm:gap-3 flex-shrink-0">
              ...
              <button class="px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-1 sm:gap-2 text-xs sm:text-sm">
                ...
                <span class="hidden sm:inline">Bulk Process</span>
      ...
      <div class="py-4 bg-white">
        <div class="max-w-[1440px] mx-auto px-3 md:px-4 lg:px-6 xl:px-8">
          ...
          <div v-else-if="filteredEmails.length === 0" class="bg-white border border-gray-200 rounded-lg p-12 text-center">
            ...
            <p class="text-gray-500">No emails found in this category</p>
          </div>
                  ...
                  >
                    <td class="px-3 sm:px-4 py-3">
                      <div class="flex items-center gap-2 sm:gap-3">
                        <div v-if="email.status === 'Unread'" class="w-2 h-2 bg-blue-500 rounded-full flex-shrink-0"></div>
                    ...
                    <td class="px-3 sm:px-4 py-3">
                      <div class="flex items-center gap-1" @click.stop="">
                        ...
                        <button class="p-1 sm:p-1.5 hover:bg-gray-100 rounded-lg transition-colors" title="More">
                          <!-- MoreVertical Icon -->
        ...
        <div class="p-6 overflow-y-auto" style="max-height: calc(90vh - 250px)">
          <div class="prose max-w-none">
            ...
            <ul class="mt-4 space-y-2">
              <li>Review all contractual obligations and ensure compliance</li>
              <li>Prepare necessary documentation for filing</li>
              <li>Coordinate with relevant parties for signatures</li>
              <li>Ensure all deadlines are met according to court requirements</li>
            </ul>
            <p class="text-gray-700 leading-relaxed mt-4">
              Please note that any delays may impact the case timeline and client expectations.
            </p>
          </div>
          ...
          <div v-if="settings.aiReadAccess" class="mt-6 p-4 bg-gray-50 rounded-xl">
            ...
            <p class="text-sm text-gray-600">
              <span v-if="selectedEmail.type === 'Contracts'">Contract review required with focus on liability and payment terms.</span>
              <span v-if="selectedEmail.type === 'Reminders'">Schedule update notification - action required to confirm availability.</span>
              <span v-if="selectedEmail.type === 'Terminations'">Employment termination agreement ready for final review and approval.</span>
              <span v-if="selectedEmail.type === 'All'">Court filing confirmation received - hearing date scheduled.</span>
            </p>
            <div class="mt-3 flex flex-wrap gap-2">
              <span class="px-2 py-1 bg-white rounded text-xs text-gray-600">Deadline: 3 days</span>
              <span class="px-2 py-1 bg-white rounded text-xs text-gray-600">Action: Review</span>
              <span class="px-2 py-1 bg-white rounded text-xs text-gray-600">Priority: {{ selectedEmail.priority }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="p-6">
          ...
          <div class="space-y-4">
            <label class="flex items-center gap-3 cursor-pointer">
              ...
              />
              <span class="text-sm text-gray-700">Allow AI read access</span>
            </label>
            ...
            <label class="flex items-center gap-3 cursor-pointer">
              <input
                ...
                class="w-4 h-4 rounded border-gray-300"
              />
              <span class="text-sm text-gray-700">Draft-only mode</span>
            </label>
            
            <div class="pt-4 space-y-2">
              <p class="text-xs text-gray-500">
                Consent timestamp: {{ settings.consentTimestamp ? settings.consentTimestamp.toLocaleString() : '—' }}
              </p>
              <p class="text-xs text-gray-500">
...
const filterTabs = computed(() => [
  { key: 'All', label: 'All' },
  { key: 'Unread', label: `Unread (${getUnreadCount()})` },
  { key: 'Reminders', label: 'Reminders' },
  { key: 'Contracts', label: 'Contracts' },
  { key: 'Terminations', label: 'Terminations' },
  { key: 'AI Pending', label: `AI Pending (${getAIPendingCount()})` }
])
...
const filteredEmails = computed(() => {
  ...
  if (activeFilter.value === 'Unread') {
    filtered = filtered.filter(e => e.status === 'Unread')
...
const formatSyncTime = () => {
  ...
  if (seconds < 10) return 'Last sync now'
...
const getUnreadCount = () => {
  return emails.value.filter(e => e.status === 'Unread').length
}
